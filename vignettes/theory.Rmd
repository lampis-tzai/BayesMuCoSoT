...
First of all we assume that the data with observations $j$ of every source $i$ are following Multivariate Normal distribution $N_p( \pmb{\theta}_i,\pmb{W}_i)$ with unknown parameters, $\pmb{\theta}_i$ the mean and $\pmb{W}_i$ the covariance matrix, or in case with predictors $N_p(\pmb{C}_{i,j}\pmb{\Theta}_i,\pmb{W}_i)$ where the $\pmb{C}_i$ be the predictors with shape n $\times$ L and $\pmb{\Theta}_i$ is the coefficient matrix (L $\times$ p). We analyze two main Bayesian approaches of data modeling: 
...
where $\pmb{\Theta}_i$ is the coefficient matrix (L $\times$ p), $\pmb{W}_i$ is the non-constant within-source covariance matrix, $\nu$ the degrees of freedom,$\pmb{U}$ the scale matrix of the inverse-Wishart distribution,  $\pmb{M}$ is the prior matrix mean (L$\times$p) and $\pmb{K}_0$ the matrix of prior measurements (L$\times$L) on the W scale, in the way that $vec(\pmb{\Theta}_i) \sim N_{l\times p}(vec(\pmb{M}),\pmb{W}_i \otimes \pmb{K}_0^{-1})$. The natural conjugate prior using the vectorized variable $\pmb{\Theta}_i$ is of the form:
...
$$    \pmb{x}_{i,j} \sim N_p(\pmb{C}_{i,j}\pmb{\Theta}_i,\pmb{W}_i)\\
    for\;l\;in\;(1,\dots,L)\\
    \pmb{\theta}_{il} \sim N_p( \pmb{\mu}_l,\pmb{B}_l)\\
    \pmb{W}_i \sim IW(\pmb{U},\nu)\\$$


where for the i-th source the $\pmb{\Theta}_i$ is the coefficient matrix (L $\times$ p), $\pmb{C}_i$ is the predictors matrix as described before with shape n $\times$ L, $\pmb{W}_{i}$ is the non-constant within-writer covariance matrix, $\pmb{\theta}_l$ is the model coefficient vector of the 'l' predictor, $\pmb{\mu}_l$ is the mean vector of the 'l' predictor between sources and $\pmb{B}_l$ is the between-sources covariance matrix of the 'l' predictor.

The marginal likelihood of this model has no analytical form. Hence, for the calculation of the marginal likelihood is needed the posterior probability distribution of the model for all the approaches in Section $\pmb{Marginal\;Likelihood\;estimations}$. Thus, for extracting sample from the posterior probability distribution it was applied Gibbs sampling by utilizing the implementation of rjags.


## Prior Elicitation

For the Bayesian modelling the prior parameters can be elicited from the background data-set with $H$ available different sources and are notated as $\pmb{x}_b$ and $\pmb{C}_b$.

$$ \pmb{M} = (\pmb{C}_b^{T}\pmb{C}_b)^{-1}\pmb{C}_b^{T}\pmb{x}_b$$
...
$$ E[\pmb{W}] = \hat{\pmb{W}}_{p \times p}  = \frac{1}{H-1}\sum_{h=1}^H  \frac{1}{n_h}(\pmb{x}_b - \pmb{C}_bM_h)(\pmb{x}_b - \pmb{C}_bM_h)^T$$
...
As we mentioned above for the independent prior approach the marginal likelihood is not known form. Hence, it is estimated by using three different marginal likelihood approaches:
